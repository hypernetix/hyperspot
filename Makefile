...
ifeq ($(OS),Windows_NT)
    QUICKSTART_CONFIG ?= config/quickstart-windows.yaml
    HYPERSPOT_SERVER_BIN := target/debug/hyperspot-server.exe
else
    QUICKSTART_CONFIG ?= config/quickstart.yaml
    HYPERSPOT_SERVER_BIN := target/debug/hyperspot-server
endif
...
openapi:
	@echo "Building hyperspot-server..."
	cargo build --bin hyperspot-server --features users-info-example
	@echo "Generating OpenAPI specs..."
	python3 scripts/openapi.py --url "$(OPENAPI_URL)" --output "$(OPENAPI_OUT)" --config "$(QUICKSTART_CONFIG)" --server-bin "$(HYPERSPOT_SERVER_BIN)"
...
quickstart:
	mkdir -p data
	cargo run --bin hyperspot-server -- --config $(QUICKSTART_CONFIG)

## Run server with example module
example:
	cargo run --bin hyperspot-server --features users-info-example,static-tenants -- --config $(QUICKSTART_CONFIG)

oop-example:
	cargo build -p calculator --features oop_module
	cargo run --bin hyperspot-server --features oop-example,users-info-example,tenant-resolver-example -- --config $(QUICKSTART_CONFIG)
