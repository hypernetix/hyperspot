...
use modkit_db::secure::{ScopableEntity, SecureConn, secure_insert};
...
async fn setup() -> (DbHandle, SecureConn, Uuid, AccessScope) {
    ...
    (db, secure, tenant_id, scope)
}

async fn seed(secure: &SecureConn, tenant_id: Uuid, scope: &AccessScope) {
    ...
    for (name, score) in rows {
        ...
        secure_insert::<ent::Entity>(am, scope, secure)
            .await
            .expect("insert");
    }
}
...
async fn paginate_odata_works_with_secure_conn() {
    let (db, secure, tenant_id, scope) = setup().await;
    seed(&secure, tenant_id, &scope).await;
    ...
    let page = OPager::<ent::Entity, _>::new(&secure, &scope, &secure, &fmap)
