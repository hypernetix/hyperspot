...
use modkit_db::secure::{
    ScopableEntity, ScopeError, SecureConn, secure_insert, secure_update_with_scope,
};
...
async fn setup() -> SecureConn {
    // Unique per-test in-memory DB to avoid parallel test locking issues.
    let test_id = Uuid::new_v4();
    let dsn =
        format!("sqlite:file:memdb_secure_update_tenant_safety_{test_id}?mode=memory&cache=shared");
    ...
    let handle = DbHandle::connect(&dsn, opts).await.expect("connect");

    let conn = handle.sea_secure();

    run_migrations_for_testing(
        &handle,
        vec![Box::new(CreateSecureUpdateTenantSafetyTables)],
    )
    .await
    .expect("migrate");
...
async fn update_many_rejects_setting_tenant_id() {
    ...
    let err = conn
        .update_many::<tenant_ent::Entity>(&scope_a)
