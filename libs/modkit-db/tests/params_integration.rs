...
async fn smoke_secure_tx(handle: &modkit_db::DbHandle) -> Result<()> {
    run_migrations_for_testing(handle, vec![Box::new(CreateParamsSmoke)])
        .await
        .map_err(|e| anyhow::anyhow!(e.to_string()))?;

    let secure = handle.sea_secure();
...
async fn pg_params_smoke() -> Result<()> {
    ...
    let handle = build_db_handle(config, None).await?;
    smoke_secure_tx(&handle).await?;
    handle.close().await;
    Ok(())
}
