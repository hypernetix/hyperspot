...
/// Run migrations for a specific module using a `DbHandle`.
///
...
/// # Arguments
///
/// * `db` - The database handle (owned by the runtime).
...
/// let result = run_migrations_for_module(&db_handle, "my_module", migrations).await?;
...
mod tests {
    ...
    mod sqlite_tests {
        use super::*;
        use crate::{ConnectOpts, DbHandle};
