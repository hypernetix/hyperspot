...
impl ClaimsPlugin for KeycloakClaimsPlugin {
    ...
    fn normalize(&self, raw: &Value) -> Result<Claims, ClaimsError> {
        ...
        // 10. Collect extra claims (excluding standard ones and Keycloak-specific fields)
        let mut extras = serde_json::Map::new();
        let keycloak_fields = ["roles", "realm_access", "resource_access"];

        if let Value::Object(obj) = raw {
            for (key, value) in obj {
                let is_standard = StandardClaim::is_registered(key);
                let is_keycloak = keycloak_fields.contains(&key.as_str());
                let is_tenant = key == &self.tenant_claim;

                if !is_standard && !is_keycloak && !is_tenant {
