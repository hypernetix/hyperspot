use modkit_bootstrap::{
    Exporter, ExporterKind, HttpOpts, LogsCorrelation, Propagation, Sampler, TracingConfig,
};
...
"#;
    ...
    assert_eq!(exporter.kind, Some(ExporterKind::OtlpGrpc));
    ...
    assert_eq!(
        cfg.sampler,
        Some(Sampler::ParentBasedRatio { ratio: Some(0.2) })
    );
...
"#;
    let cfg: TracingConfig = serde_saphyr::from_str(yaml).unwrap();
    let exporter = cfg.exporter.as_ref().unwrap();
    assert_eq!(exporter.kind, Some(ExporterKind::OtlpHttp));
    assert_eq!(
...
"#;
    let cfg: TracingConfig = serde_saphyr::from_str(yaml).unwrap();
    assert_eq!(cfg.sampler, Some(Sampler::AlwaysOn {}));
...
"#;
    let cfg: TracingConfig = serde_saphyr::from_str(yaml).unwrap();
    assert_eq!(cfg.sampler, Some(Sampler::AlwaysOff {}));
...
"#;
    ...
    assert_eq!(
        cfg.sampler,
        Some(Sampler::ParentBasedRatio { ratio: Some(0.5) })
    );
}
...
fn test_config_serialization_roundtrip() {
    ...
    assert_eq!(original.sampler.unwrap(), roundtrip.sampler.unwrap());
    assert_eq!(
