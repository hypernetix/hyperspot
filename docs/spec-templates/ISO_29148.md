...
**Fulfillment:** Functional requirements are clearly structured with MUST language, IDs, rationale, source, and actor mapping, and are traceable into design and feature specs. This is stronger than typical 29148 usage because it embeds traceability and implementation status inline. Verification method is implicit (default: automated tests with 95% coverage) and only documented explicitly for exceptions (analysis, inspection, demonstration), which is pragmatic and reduces documentation overhead without sacrificing ISO alignment.
...
**Deviations / possible future improvements:** verification method and evidence are not required fields for every requirement. Require a per-requirement verification method + evidence link, and have CI enforce that tests reference the relevant `fdd-...` IDs for implemented requirements.
...
| **Functional Requirements** | IDs, rationale, source, actors; implicit testing standard (95% coverage) |
