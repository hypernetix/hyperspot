...
Industry-standard specification templates incorporating best practices from IEEE, ISO, and modern software development methodologies. Compatible with [Spider-Driven Development](https://github.com/cyberfabric/Spider) for validation and traceability.
...
Templates work standalone or can be enhanced with Spider annotations (`spd-id`) for cross-document validation and traceability.

**Spider integration is optional** — templates are useful on their own for clear, consistent documentation.
...
Spider IDs enable traceability across all specification artifacts.

### Spider ID Definition

An Spider ID **defines** a unique identifier for a specification element (actor, requirement, feature, etc.). Each ID must be **globally unique** within the module, subsystem or global project depending on where it's defined
...
An Spider ID covers the **markdown section where it's defined and all its subsections**:
...
An Spider ID **reference** links to an element defined elsewhere. References create traceability between documents — for example, a Feature can reference Actors from PRD, or an ADR can reference Requirements it addresses.

**Placement**: Use backtick notation `` `spd-...` `` when referencing an ID defined in another section or file.

### Validation

Spider IDs must be unique. When Spider tooling is connected, `spider validate` will:
...
These are **suggested** kind names for common artifact types. Spider does not enforce specific kind values — use whatever naming makes sense for your project. The important thing is consistency within your codebase.
...
> **Note**: You can use any slug that fits your domain. For example, `spd-billing-uc-checkout` or `spd-auth-nfr-token-expiry` are equally valid if your team prefers more specific kinds. Spider validation only checks that referenced IDs exist — it does not validate kind names.
...
When full Spider framework is connected:

1. **Validation** — `spider validate` will check document structure and cross-references
...
For more details on Spider taxonomy and artifact relationships, see [`TAXONOMY.md`](https://github.com/cyberfabric/Spider/blob/main/guides/TAXONOMY.md).
