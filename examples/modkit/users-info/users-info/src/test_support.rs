...
/// Mock `AuthZ` resolver that explicitly denies every request.
///
/// Returns `AuthZResolverError::Denied` — simulates a PDP that rejects access
/// outright (as opposed to returning empty constraints).
...
/// `EnforcerError::Denied` (not `EvaluationFailed`).
pub struct DecisionDeniedAuthZResolver;

#[async_trait]
impl AuthZResolverClient for DecisionDeniedAuthZResolver {
    async fn evaluate(
        &self,
        _request: EvaluationRequest,
    ) -> Result<EvaluationResponse, AuthZResolverError> {
...
impl AuthZResolverClient for OwnerCityAuthZResolver {
    async fn evaluate(
        ...
        let mut predicates = vec![Predicate::In(InPredicate {
            property: pep_properties::OWNER_TENANT_ID.to_owned(),
            values: vec![root_id],
        })];
        ...
        if is_address && is_mutation {
            ...
            predicates.push(Predicate::Eq(EqPredicate {
                property: pep_properties::OWNER_ID.to_owned(),
