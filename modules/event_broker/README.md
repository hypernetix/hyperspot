...
- **Sequential ordering**: Events are ordered by monotonically increasing sequence numbers
- **GTS-typed events**: All events are identified by GTS types identifiers
- **Immutable log**: Events are append-only with guaranteed ordering
- **Cursor-based pagination**: Efficient traversal of event streams
- **Long-polling support**: Reduce polling overhead with wait-for-data semantics
...
```json
{
  "id": "gts.hyperspot.core.events.topic.v1~",
...
```bash
GET /v1/topics?topic=gts.hyperspot.core.events.topic.v1~vendor.users.v1
```
...
```json
{
  "topics": [
    {
      "id": "gts.hyperspot.core.events.topic.v1~vendor.users.v1",
      "description": "User-related events",
      "streaming": {
        "type": "file",
        "location": "file://var/lib/hyperspot/events",
...
```bash
GET /v1/topics/segments?topic=gts.hyperspot.core.events.topic.v1~vendor.users.v1
```
...
```json
{
  "topic": "gts.hyperspot.core.events.topic.v1~vendor.users.v1",
  "start": 123456,
...
```bash
GET /v1/events?topic=gts.hyperspot.core.events.topic.v1~&offset=10000&limit=50
```
...
```bash
GET /v1/events:poll?topic=gts.hyperspot.core.events.topic.v1~&offset=12345&timeout=30&limit=25
```
...
```json
{
  ...
  "topic": "gts.hyperspot.core.events.topic.v1~",
  "types": [
...
This design aligns with event streaming semantics used in systems like Kafka.

- **Offset = Sequence Number**: The `offset` parameter represents the last seen event sequence number
- **Deterministic Ordering**: Events are ordered by monotonically increasing `sequence` field
