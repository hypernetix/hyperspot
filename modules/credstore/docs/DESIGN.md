...
DESIGN LANGUAGE:
  ...
  - Reference PRD requirements using `cpt-cf-credstore-req-{slug}` IDs
...
|-------------|-----------------|
| `cpt-cf-credstore-req-put-secret` | Plugin `put` with tenant_id, key, value, sharing → backend storage |
| `cpt-cf-credstore-req-get-secret` | Plugin `get` with tenant_id, key, optional owner_id → backend lookup (two-phase: private then tenant/shared) |
| `cpt-cf-credstore-req-delete-secret` | Plugin `delete` with tenant_id, key, optional owner_id → backend removal |
| `cpt-cf-credstore-req-tenant-scoping` | Gateway extracts tenant_id from SecurityCtx before delegating to plugin |
| `cpt-cf-credstore-req-sharing-modes` | `sharing` field in Credstore backend (VendorA); passed through from Gateway API |
| `cpt-cf-credstore-req-authz-gateway` | Gateway checks SecurityCtx permissions before any plugin call |
| `cpt-cf-credstore-req-rw-separation` | VendorA plugin configures separate RO/RW OAuth2 client credentials |

#### NFR Allocation

| NFR ID | NFR Summary | Allocated To | Design Response | Verification Approach |
|--------|-------------|--------------|-----------------|----------------------|
| `cpt-cf-credstore-req-nfr-confidentiality` | Secret values never in logs | Gateway + plugins | `SecretValue` wrapper type with custom `Debug`/`Display` that redacts content; log scrubbing at transport layer | Automated log scan in integration tests |
...
- NFR requirement (`cpt-cf-credstore-req-nfr-confidentiality`)
