...
### 1.4 Update Handlers
- [x] 1.4.1 Prepare handlers for SecurityCtx delegation pattern
  - **Implementation Note**: GTS Core routing layer handlers do NOT need `Extension<SecurityCtx>` in signatures during routing-only phase
  - **Rationale**: SecurityCtx is provided by api_gateway and will be passed to domain features when delegation is implemented
  - **Architecture**: Per DESIGN.md lines 665-686, "GTS Core injects SecurityCtx into all domain feature calls" (not handler signatures)
  - **Current**: Handlers return NOT_IMPLEMENTED (architectural stub per feature-status-validation.md)
  - **Future**: When domain features exist, handlers will extract SecurityCtx and pass to `domain_feature.handle(ctx, request)`
  - **Project Pattern**: Use `Authz` extractor when implementing delegation (see file_parser module)
...
- [x] 2.1.5 Verify handlers prepared for future SecurityCtx delegation (architectural stub pattern - NOT in signatures yet)
