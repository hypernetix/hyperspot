...
impl TenantResolverPluginClient for Service {
    ...
    async fn can_access(
        ...
        // Note: Self-access is handled by gateway, not plugin
    ...
    async fn get_accessible_tenants(
        ...
        // Note: Self-tenant is added by gateway, not plugin
...
mod tests {
    ...
    #[tokio::test]
    async fn can_access_self_not_handled_by_plugin() {
        // Plugin does NOT handle self-access (gateway does)
        let cfg = StaticTrPluginConfig {
            tenants: vec![tenant(TENANT_A, "A", TenantStatus::Active)],
            access_rules: vec![], // No explicit self-access rule
            ..Default::default()
        };
        let service = Service::from_config(&cfg);
        let ctx = ctx_for_tenant(TENANT_A);

        // Plugin returns false for self-access (gateway adds this logic)
        let result = service
    ...
    async fn get_accessible_tenants_filtered_by_status() {
        ...
        // With active filter - returns only B (C is suspended)
