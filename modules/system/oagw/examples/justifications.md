...
### [x] 3.9 Justification: Create AMQP route by exchange+routing_key
- Why it matters:
  - AMQP does not use HTTP path/method matching.
- What to check:
  - `match.amqp.exchange` + `match.amqp.routing_key` matches publish requests.
  - Invalid routing key pattern rejected at config validation.
...
## 16) Protocol coverage: AMQP

### [x] 16.1 Justification: AMQP publish route
- Why it matters:
  - Message-based integrations.
- What to check:
  - Route matches on exchange + routing key.
  - Message published with correct properties.

### [x] 16.2 Justification: AMQP auth and secret handling
- Why it matters:
  - Broker credentials must be protected.
- What to check:
  - Credentials injected from `cred_store`.
  - Access denied returns gateway `401`.

### [x] 16.3 Justification: AMQP error source distinction
- Why it matters:
  - Broker errors vs gateway errors.
- What to check:
  - Connection refused produces gateway error (`502/503`, `ESrc=gateway`).
  - Broker negative-ack (if modeled) marked as upstream.
...
- `noop`: HTTP
- `apikey`: HTTP, WebSocket, SSE, gRPC, AMQP
- `basic`: HTTP
- `bearer`: HTTP
