...
/// Trait for types that can be used as resource property values.
///
/// Enables ergonomic property value conversions without requiring explicit
/// `serde_json::json!()` wrapping or `.to_string()` calls.
///
/// # Examples
///
/// ```ignore
/// use uuid::Uuid;
/// use authz_resolver_sdk::pep::{AccessRequest, IntoPropertyValue};
///
/// let tenant_id = Uuid::new_v4();
/// let req = AccessRequest::new()
///     .resource_property("owner_tenant_id", tenant_id);  // Auto-converts
/// ```
pub trait IntoPropertyValue {
    /// Convert this value into a `serde_json::Value` for authorization evaluation.
    fn into_property_value(self) -> serde_json::Value;
}

// Primary implementation: Uuid (100% of current usage)
...
// Fallback for raw serde_json::Value
impl IntoPropertyValue for serde_json::Value {
    #[inline]
    fn into_property_value(self) -> serde_json::Value {
        self
    }
...
impl AccessRequest {
    ...
    pub fn resource_property(
        ...
        self.resource_properties
            .insert(key.into(), value.into_property_value());
        self
    }
