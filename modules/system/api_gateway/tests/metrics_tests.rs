...
async fn test_metrics_histogram_present() {
    ...
    // Should contain histogram metrics with correct metric name
    // Metric name: hyperspot_http_requests_duration_seconds (using server.app_name as prefix)
    assert!(
        body_str.contains("hyperspot_http_requests_duration_seconds"),
        "Metrics should contain the correct histogram metric name"
    );
}
