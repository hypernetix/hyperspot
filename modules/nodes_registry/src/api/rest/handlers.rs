...
pub async fn list_nodes(
    ...
    let nodes = svc.list_nodes().await?;
...
pub async fn get_node(
    ...
    if query.details {
        let sysinfo = svc.get_node_sysinfo(id).await.ok().map(Into::into);
        let syscap = svc
...
pub async fn get_node_sysinfo(
    ...
    let sysinfo = svc.get_node_sysinfo(node_id).await?;
...
pub async fn get_node_syscap(
    ...
    let syscap = svc.get_node_syscap(node_id, query.force_refresh).await?;
    Ok(Json(syscap.into()))
}
