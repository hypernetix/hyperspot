...
- Provide integrated retry and error normalization for known protocols.
...
- `gts.x.core.oagw.proto.v1~x.core.grpc.abc.v1`
...
- gRPC (`gts.x.core.oagw.proto.v1~x.core.grpc.grpc.v1`) as a typed gRPC stream.
...
For outbound gRPC (`grpc`) the gateway SHOULD use:
- `tonic` for gRPC client generation and streaming support.

#### Scenario: Transport selected by protocol
- **GIVEN** a link requires `protocol_gts_id` = `gts.x.core.oagw.proto.v1~x.core.grpc.grpc.v1`
- **WHEN** `oagw` executes the invocation
- **THEN** it uses a gRPC transport implementation
...
### Requirement: Integrated retry and error handling
The system SHALL implement protocol-aware retry and error normalization in the gateway.

#### Scenario: Retry transient errors
- **WHEN** an invocation fails with a transient transport error
- **THEN** the gateway retries according to the selected strategy
- **AND** produces a consistent error response if retries are exhausted
...
- `operation text not null` (HTTP method or gRPC method)
- `target_url text not null`
- `status_code int null` (HTTP status or gRPC status code)
...
- `oagw.invocations` (counter, labels: tenant_id, route_id, status)
- `oagw.request.duration` (histogram, labels: route_id, protocol)
- `oagw.errors` (counter, labels: error_type, route_id)
...
- `oagw.bytes_sent` (counter, labels: tenant_id, route_id, link_id)
- `oagw.bytes_received` (counter, labels: tenant_id, route_id, link_id)
- `oagw.token_cache.hit_rate` (gauge)
...
- `GET /oagw/v1/ready` - Readiness probe (checks no circuit breakers open)
- `GET /oagw/v1/routes/{routeId}/health` - Route-specific health check

The health check endpoints access must be protected by role `oagw.health_check` assigned.

#### Scenario: Check downstream health
- **WHEN** `GET /oagw/v1/routes/{routeId}/health` is called
- **THEN** access is protected by role `oagw.health_check` assigned
- **AND** system tests connectivity to all links for that route
- **AND** returns aggregated health status

#### Scenario: Readiness check with circuit breaker
- **WHEN** `GET /oagw/v1/ready` is called
- **AND** access is protected by role `oagw.health_check` assigned
- **AND** at least one link has open circuit breaker
- **THEN** returns 503 Service Unavailable
- **AND** includes details about affected links
...
The system SHALL implement circuit breaker per link to prevent cascade failures.
...
#### Scenario: gRPC stream error
- **WHEN** gRPC stream returns error after N messages
- **THEN** gateway propagates gRPC status code to client
- **AND** ensures proper stream cleanup (no resource leak)
