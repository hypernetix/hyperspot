...
- Use when:
  - Module has 6+ aggregates with complex operations
...
```rust
// src/infra/storage/users_sea_repo.rs
use async_trait::async_trait;
use modkit_db::secure::SecureConn;
use modkit_security::AccessScope;
...
**Note:** The `examples/modkit/users_info/` module uses the per-aggregate pattern for demonstration purposes.
When creating new modules, prefer the single-file `src/infra/storage/repo.rs` pattern shown in this guide
unless your module specifically needs the per-aggregate approach.
